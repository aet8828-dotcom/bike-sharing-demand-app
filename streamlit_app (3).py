...
""")
    ...
    season = df.groupby("Season_Name", as_index=False)["cnt"].mean()
...
##Person 3: Saveena Boga
    ...
    ## Loading model data based on selection
    ...
    if model_choice == "Linear Regression":
        ...
        **Reading this chart:** A good model has residuals scattered randomly around 0.
        Linear Regression shows a clear fan-shaped pattern — errors get larger at
    ...
    **Conclusion:** Random Forest outperforms Linear Regression across all three metrics.
    It achieves an R² of 0.928, meaning it explains 93% of the variation in hourly
...
## Person 4
    ...
    with tab1:
        ...
        if lr_path.exists():
            ...
            st.markdown("""
**Interpretation:**
- **Hour (hr)** has the largest positive effect — demand rises sharply during commuting hours.
- **Temperature (atemp/temp)** increases demand — warmer conditions encourage riding.
- **Humidity (hum)** and **windspeed** reduce demand — uncomfortable conditions deter riders.
...
""")
    ...
    with tab2:
        ...
        if image_path.exists():
            ...
            fig, ax = plt.subplots(figsize=(9, 6))
            ax.barh(importance_df["Feature"][::-1], importance_df["Importance"][::-1], color="#3498db")
        ...
        st.markdown("""
**Interpretation:**
- **Hour of day (hr)** is the strongest predictor — demand spikes at 8 AM and 5–6 PM.
- **Apparent temperature (atemp)** is second most important — rider comfort drives usage.
...
## Person 4
    ...
    if not lr_path.exists() or not rf_path.exists():
        ...
        with col1:
            n_est_opts = st.multiselect("n_estimators", options=[50, 100, 200, 300], default=[100, 200])
            depth_opts = st.multiselect("max_depth", options=[10, 15, 20, 25], default=[15, 20])
        with col2:
            split_opts = st.multiselect("min_samples_split", options=[2, 5, 10], default=[2, 5])
            leaf_opts  = st.multiselect("min_samples_leaf", options=[1, 2, 4], default=[1, 2])

        if not all([n_est_opts, depth_opts, split_opts, leaf_opts]):
            st.warning("Please select at least one value for each parameter.")
        else:
            param_grid = list(ParameterGrid({
                "n_estimators": n_est_opts, "max_depth": depth_opts,
                "min_samples_split": split_opts, "min_samples_leaf": leaf_opts,
            }))
            ...
            if st.button("Run Tuning"):
                ...
                for i, params in enumerate(param_grid):
                    ...
                    results.append({"Experiment": i+1, **params, **get_metrics(m, X_test_t, y_test)})
                ...
                c3.metric("Best R²", best["R2"])
                st.json({"n_estimators": int(best["n_estimators"]), "max_depth": int(best["max_depth"]),
                         "min_samples_split": int(best["min_samples_split"]), "min_samples_leaf": int(best["min_samples_leaf"])})

                final_df = pd.DataFrame([
                    {"Model": "Linear Regression", **lr_metrics},
                    {"Model": "Random Forest (baseline)", **rf_metrics},
                    {"Model": "Random Forest (tuned)", "MAE": best["MAE"], "RMSE": best["RMSE"], "R2": best["R2"]},
                ])
                ...
                ax.bar(final_df["Model"], final_df["MAE"], color=["#e74c3c", "#3498db", "#2ecc71"])
                ...
                ax2.axhline(rf_metrics["MAE"], color="#e74c3c", linestyle="--", label="Baseline RF MAE")
...
If they are similar, keep the original Random Forest. Share final_model_comparison.csv with Person 1.
""")
